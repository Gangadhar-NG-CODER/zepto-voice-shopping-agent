...
function extractCartFromSummary(message: string): CartItem[] {
  ...
  // Pattern: "2 Whole Wheat Bread at rupees 45.0 each" or "2 Whole Wheat Bread at rupees 45 each"
  // Split by commas and "and" first, then parse each segment
  const cartContentMatch = message.match(/items in your cart:\s*(.+?)\.\s*Your total/i);
  
  if (!cartContentMatch) {
    console.log('🔍 Could not find cart content in message');
    return items;
  }
  
  const cartContent = cartContentMatch[1];
  console.log('🔍 Parsing cart content:', cartContent);
  
  // Split by comma or "and" to get individual items
  const itemSegments = cartContent.split(/,\s*(?:and\s+)?|\s+and\s+/);
  
  itemSegments.forEach((segment) => {
    // Pattern: "2 Whole Wheat Bread at rupees 45.0 each"
    const itemMatch = segment.match(/(\d+)\s+(.+?)\s+at\s+rupees\s+(\d+(?:\.\d+)?)\s+each/i);
    ...
    if (itemMatch) {
      ...
      console.log(`🔍 Found item: ${quantity}x ${name} @ ₹${price}`);
